...
stable_number(struct table *t, const char *key, size_t sz_idx) {
	struct value tmp;
	_search_table(t,key,sz_idx,&tmp);
	assert(tmp.type == ST_NIL || tmp.type == ST_NUMBER || tmp.type == ST_INTGER);

	if (tmp.type == ST_INTGER)
	{
		return tmp.v.inter;
	}
...
stable_setnumber(struct table *t, const char *key, size_t sz_idx, double n) {
	long long inter = 0;
	struct value tmp;
	if (lua_numbertointeger(n, &inter))
	{
		tmp.type = ST_INTGER;
		tmp.v.inter = inter;
	}
	...
	int type = _insert_table(t,key,sz_idx,&tmp);
	return type != ST_NUMBER && type != ST_NIL && type != ST_INTGER;
}
