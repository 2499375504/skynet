...
LUA_CLIB = skynet \
  client \
  bson md5 sproto lpeg stable int64 $(TLS_MODULE)
...
SKYNET_SRC = skynet_main.c skynet_handle.c skynet_module.c skynet_mq.c \
  ...
  malloc_hook.c skynet_daemon.c skynet_log.c stable.c
...
$(LUA_CLIB_PATH)/stable.so : lualib-src/lua-stable.c| $(LUA_CLIB_PATH)
	$(CC) $(CFLAGS) $(SHARED) -Iskynet-src $^ -o $@	

$(LUA_CLIB_PATH)/int64.so : lualib-src/lua-int64.c| $(LUA_CLIB_PATH)
	$(CC) $(CFLAGS) $(SHARED) -Iskynet-src $^ -o $@
